...
//
  ...
  if ( iflag != 1 )
  {
    if ( t != told )
    {
      ...
      qDebug() << "\n";
      qDebug() << "DE - Fatal error!\n";
      qDebug() << "  IFLAG is not 1, and T is not equal to TOLD.\n";
      exit ( 1 );
    }
...
//
    ...
    {
      iflag = isn * 3;
 //   relerr = eps * releps;                 Oleg comment  --- may be change argument list?
 //   abserr = eps * abseps;
      for ( l = 1; l <= neqn; l++ )
      {
        y[l-1] = yy[l-1];
