 ...
 *
 * I have removed quite a lot of text/comments from this library during
 development.
 * Please refer to the standard documentation.
 *
 * Contact: Dilawar Singh <dilawar_s@zohomail.in>
*/
...
LSODA::LSODA()
{
    // Initialize arrays.
    mord = { { 12, 5 } };
    sm1 = { { 0., 0.5, 0.575, 0.55, 0.45, 0.35, 0.25, 0.2, 0.15, 0.1, 0.075,
        0.05, 0.025 } };
    el = { { 0 } };
    cm1 = { { 0 } };
    cm2 = { { 0 } };
...
bool LSODA::abs_compare(real a, real b)
{
...
size_t LSODA::idamax1(
    const vector<real>& dx, const size_t n, const size_t offset = 0)
{
...
/* Purpose : scalar vector multiplication
   dx = da * dx
*/
void LSODA::dscal1(const real da, vector<real>& dx, const size_t n,
                   const size_t offset)
{
    for (size_t i = 1; i <= n; i++)
        dx[i + offset] *= da;
...
real LSODA::ddot1(const vector<real>& a, const vector<real>& b,
    const size_t n, const size_t offsetA = 0, const size_t offsetB = 0)
{
    real sum = 0.0;
    for (size_t i = 1; i <= n; i++)
...
void LSODA::daxpy1(const real da, const vector<real>& dx,
                   vector<real>& dy, const size_t n, const size_t offsetX = 0,
...
void LSODA::dgesl(const vector<vector<real>>& a, const size_t n,
...
void LSODA::dgefa(vector<vector<real>>& a, const size_t n, vector<int>& ipvt,
...
void LSODA::dgbfa(vector<vector<real>>& a, const size_t lda, const size_t n,
                  const size_t ml, const size_t mu, vector<int>& ipvt, size_t* info)
{
    size_t i, j, k, l, lm, ju, jz, m;
    real t;
    ...
    *info = 0;
    ...
    for (k = 1; k <= n - 1; k++) {
        jz = min(k + mu, n);
        lm = min(ml, n - k);
        l = idamax1(a[m], lm + 1, k - 1) + k - 1;
        ipvt[k] = l;
        if (a[m][l] != 0.0) {
            if (l != k) {
                for (j = 0; j <= min(jz - k, m - 1); j++) {
                    t = a[m - j][l];
                    a[m - j][l] = a[m - j][k];
                    a[m - j][k] = t;
                }
...
// Banded back substitution (adapted from LINPACK dgbsl)
void LSODA::dgbsl(vector<vector<real>>& a, const size_t lda, const size_t n,
                  const size_t ml, const size_t mu, vector<int>& ipvt, vector<real>& b,
                  const size_t job)
{
    size_t k, l, la, lb, lm, m;
    real t;
    ...
    if (job == 0) {
        for (k = 1; k <= n - 1; k++) {
            ...
            for (la = m + 1, lb = k + 1; la <= m + lm; la++, lb++)
                b[lb] += t * a[la][k];
        }
        for (k = n; k >= 1; k--) {
            b[k] /= a[m][k];
...
void LSODA::terminate2(vector<real>& y, real* t)
...
void LSODA::successreturn(vector<real>& y, real* t, int itask, int ihit,
...
c-----------------------------------------------------------------------
*/
void LSODA::lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<real>& y,
                  ...
                  array<int, 7>& iworks, array<real, 4>& rworks, void* _data)
{
    ...
    real atoli = 0, ayi = 0, big = 0, h0 = 0, hmax = 0, hmx = 0, rh = 0,
    ...
    */
    ...
    if (*istate == 1 || *istate == 3) {
        ...
        if (jt > 2) {
            ...
            mu = iworks[1];
            if (ml >= n) {
                cerr << "[lsoda] ml = " << ml << " not between 1 and neq"
                     << endl;
                terminate(istate);
                return;
            }
            if (mu >= n) {
                cerr << "[lsoda] mu = " << mu << " not between 1 and neq"
                     << endl;
                terminate(istate);
                return;
            }
    ...
    if (*istate == 1) {
        ...
        if (itask == 4 || itask == 5) {
            ...
            if ((tcrit - tout) * (tout - *t) < 0.) {
                fprintf(
                    stderr, "[lsoda] itask = 4 or 5 and tcrit behind tout\n");
                terminate(istate);
                return;
            }
        ...
        */
        if (h0 == 0.) {
            ...
            tol = max(tol, static_cast<real>(100.) * ETA);
            tol = min(tol, 0.001);
    ...
    */
    if (*istate == 2 || *istate == 3) {
        ...
        case 2:
            break;
        case 3:
            tp = tn_ - hu * (1. + static_cast<real>(100.) * ETA);
            if ((tp - tout) * h_ > 0.) {
                fprintf(stderr,
        ...
        case 4:
            ...
            if ((tn_ - tcrit) * h_ > 0.) {
                fprintf(
                    stderr, "[lsoda] itask = 4 or 5 and tcrit behind tcur\n");
                terminate(istate);
                return;
            }
            if ((tcrit - tout) * h_ < 0.) {
        ...
        case 5:
            ...
            ihit = fabs(tn_ - tcrit) <= (static_cast<real>(100.) * ETA * hmx);
            if (ihit) {
                *t = tcrit;
                successreturn(y, t, itask, ihit, tcrit, istate);
                return;
    ...
    while (1) {
        if (*istate != 1 || nst != 0) {
            if ((nst - nslast) >= mxstep) {
                cerr << "[lsoda] " << mxstep
                     << " steps taken before reaching tout" << endl;
                *istate = -1;
                terminate2(y, t);
                return;
            }
        ...
        if (tolsf > 0.01) {
            ...
            if (nst == 0) {
                ...
                fprintf(
                    stderr, "         requested for precision of machine,\n");
                fprintf(
                    stderr, "         suggested scaling factor = %g\n", tolsf);
                terminate(istate);
                return;
            }
        ...
        if ((tn_ + h_) == tn_) {
            ...
            if (nhnil <= mxhnil) {
                fprintf(stderr,
                    ...
                    h_);
                fprintf(stderr,
                    "         such that in the machine, t + h_ = t on the next "
                    "step\n");
        ...
        if (kflag == 0) {
            ...
            */
            ...
            if (meth_ != mused) {
                ...
                if (ixpr) {
                    if (meth_ == 2)
                        cerr << "[lsoda] a switch to the stiff method has "
                                "occurred "
                             << endl;
                    if (meth_ == 1)
                        cerr << "[lsoda] a switch to the nonstiff method has "
                                "occurred"
                             << endl;
                }
            } /* end if ( meth_ != mused )   */
...
void LSODA::stoda(
    const size_t neq, vector<real>& y, LSODA_ODE_SYSTEM_TYPE f, void* _data)
{
    assert(neq + 1 == y.size());

    size_t corflag = 0, orderflag = 0;
    size_t i = 0, i1 = 0, j = 0, m = 0, ncf = 0;
    real del = 0.0, delp = 0.0, dsm = 0.0, dup = 0.0, exup = 0.0, r = 0.0,
           rh = 0.0, rhup = 0.0, told = 0.0;
    real pdh = 0.0, pnorm = 0.0;
...
void LSODA::ewset(const vector<real>& ycur)
{
    switch (itol_) {
    case 1:
        for (size_t i = 1; i <= n; i++)
...
*/
void LSODA::intdy(real t, int k, vector<real>& dky, int* iflag)
{
    int ic, jp1 = 0;
    real c, r, s, tp;
    ...
    *iflag = 0;
    ...
    tp = tn_ - hu - static_cast<real>(100.) * ETA * (tn_ + hu);
    if ((t - tp) * (t - tn_) > 0.) {
        fprintf(stderr,
...
{
    int i, nq, nqm1, nqp1;
    real agamq, fnq, fnqm1, pc[13], pint, ragq, rqfac, rq1fac, tsign, xpin;
    /*
    ...
    */
    if (meth_ == 1) {
        ...
        for (nq = 2; nq <= 12; nq++) {
            ...
            */
            pint += tsign * pc[i] / (real)i;
            xpin += tsign * pc[i] / (real)(i + 1);
            tsign = 1.;
            for (i = 2; i <= nq; i++) {
                tsign = -tsign;
            ...
            */
            ...
            elco[nq][2] = 1.;
            for (i = 2; i <= nq; i++)
                elco[nq][i + 1] = rq1fac * pc[i] / (real)i;
            agamq = rqfac * xpin;
            ragq = 1. / agamq;
            tesco[nq][2] = ragq;
            if (nq < 12)
                tesco[nqp1][1] = ragq * rqfac / (real)nqp1;
            tesco[nqm1][3] = ragq;
        } /* end for   */
        return;
    } /* end if ( meth_ == 1 )   */
    ...
    */
    for (nq = 1; nq <= 5; nq++) {
        ...
        */
        ...
        elco[nq][2] = 1.;
        tesco[nq][1] = rq1fac;
        tesco[nq][2] = ((real)nqp1) / elco[nq][1];
        tesco[nq][3] = ((real)(nq + 2)) / elco[nq][1];
        rq1fac /= fnq;
    }
    return;
...
void LSODA::scaleh(real* rh, real* pdh)
{
    real r;
    /*
    ...
    const size_t neq, vector<real>& y, LSODA_ODE_SYSTEM_TYPE f, void* _data)
{
    ...
    real fac = 0.0, hl0 = 0.0, r = 0.0, r0 = 0.0, yj = 0.0;
    /*
    ...
    */
    if (miter != 2 && miter != 5) {
        fprintf(stderr, "[prja] unsupported miter = %zu\n", miter);
        return;
    }
    if (miter == 2) {
        fac = vmnorm(n, savf, ewt);
        r0 = 1000. * fabs(h_) * ETA * ((real)n) * fac;
        if (r0 == 0.)
            r0 = 1.;
        for (j = 1; j <= n; j++) {
            yj = y[j];
        ...
        */
        ...
        for (j = 1; j <= n; j++) {
            ...
            fac = -hl0 / r;
            (*f)(tn_, &y[1], &acor[1], _data);
            for (i = max((size_t)1, j - mu); i <= min(n, j + ml); i++)
                wm_[mu + i - j + 1][j] = (acor[i] - savf[i]) * fac;
            y[j] = yj;
        }
        nfe += n;
        pdnorm = fnorm(n, wm_, ewt) / fabs(hl0);
        for (j = 1; j <= n; j++)
            wm_[mu + 1][j] += 1.0;
        dgbfa(wm_, ml + mu + 1, n, ml, mu, ipvt, &ier);
        if (ier != 0)
            ierpj = 1;
        return;
    }
...
*/
void LSODA::correction(const size_t neq, vector<real>& y,
                       LSODA_ODE_SYSTEM_TYPE f, size_t* corflag, real pnorm, real* del,
                       real* delp, real* told, size_t* ncf, real* rh, size_t* m, void* _data)
{
    real rm = 0.0, rate = 0.0, dcon = 0.0;
...
void LSODA::corfailure(real* told, real* rh, size_t* ncf, size_t* corflag)
{
    ncf++;
    rmax = 2.;
    tn_ = *told;
...
/*
   ...
   y = the right-hand side vector on input, and the solution vector
       on output.
*/
void LSODA::solsy(vector<real>& y)
{
    iersl = 0;
    if (miter == 2) {
        dgesl(wm_, n, ipvt, y, 0);
    } else if (miter == 5) {
        dgbsl(wm_, ml + mu + 1, n, ml, mu, ipvt, y, 0);
    } else {
        printf("solsy -- unsupported miter %zu\n", miter);
        return;
    }
    return;
}

void LSODA::methodswitch(real dsm, real pnorm, real* pdh, real* rh)
{
...
{
    size_t newq = 0;
    real exsm, rhdn, rhsm, ddn, exdn, r;
...
void LSODA::lsoda_update(LSODA_ODE_SYSTEM_TYPE f, const size_t neq,
                         vector<real>& y, vector<real>& yout, real* t, const real tout,
                         int* istate, void* _data, real rtol, real atol)
{
    array<int, 7> iworks = { { 0 } };
    array<real, 4> rworks = { { 0.0 } };
