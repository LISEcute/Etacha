...
# include "o_ODE/rkf45.hpp"

extern int ODEsteps;
extern int ODEmaxnum;
...
//
  ...
  if ( neqn < 1 )
  {
    flag_return = 8;
    qDebug() << "\n";
    qDebug() << "R8_RKF45 - Fatal error!\n";
    qDebug() << "  Invalid input value of NEQN.\n";
    return flag_return;
  }
  ...
  if ( (*relerr) < 0.0 )
  {
    ...
    qDebug() << "\n";
    qDebug() << "R8_RKF45 - Fatal error!\n";
    qDebug() << "  Invalid input value of RELERR.\n";
    return flag_return;
  }
  ...
  if ( abserr < 0.0 )
  {
    ...
    qDebug() << "\n";
    qDebug() << "R8_RKF45 - Fatal error!\n";
    qDebug() << "  Invalid input value of ABSERR.\n";
    return flag_return;
  }
...
//
    if ( mflag == 2 )
    {
      ...
      else if ( kflag == 6 && (*relerr) <= relerr_save && abserr <= abserr_save )
      {
        ...
        qDebug() << "R8_RKF45 - Fatal error!\n";
        qDebug() << "  KFLAG = 6 and\n";
        qDebug() << "  RELERR <= RELERR_SAVE and\n";
        qDebug() << "  ABSERR <= ABSERR_SAVE\n";
        exit ( 1 );
      }
    }
//
...
//
      else
      {
        qDebug() << "\n";
        qDebug() << "R8_RKF45 - Fatal error!\n";
        qDebug() << "  Integration cannot be continued.\n";
        qDebug() << "  The user did not respond to the output\n";
        qDebug() << "  value FLAG = 5, 6, 7, or 8.\n";
        exit ( 1 );
      }
    }
  }
